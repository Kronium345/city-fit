  ...
  return (
    <View style={{ flex: 1 }}>
        ...
        {messages.map((message, index) => (
          <View key={index} style={{ flexDirection: message.sender === 'user' ? 'row-reverse' : 'row' }}>
            <View
              ...
              <Text style={{ color: message.sender === 'user' ? 'white' : 'black' }}>{message.message}</Text>
            </View>
          </View>
        ))}
        {typing && (
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10 }}>
        <TextInput
          style={{ flex: 1, borderWidth: 1, borderColor: 'gray', borderRadius: 10, padding: 10, maxWidth: '85%' }}
          value={inputMessage}
